		...
		while (continuar_demanant == 1) {
			try {
				...
				if (menu.containsKey(nom_plat)) {
					...
					throw new RuntimeException("El plat no está disponible al menú");
			} catch (Exception e) {
			...
			while (cont_demanant_ok == 0) {
				try {
					...
					if (continuar_demanant!=0 && continuar_demanant!=1) 
						throw new ExcepcioContinuarDemanant("La resposta no és correcta, ha de ser o 0 o 1");
				} catch (NumberFormatException e) {
					System.out.println("La resposta no és correcta, ha de ser o 0 o 1");
					cont_demanant_ok=0;
				} catch (ExcepcioContinuarDemanant e) {
					System.out.println(e.getMessage());
					cont_demanant_ok=0;
				}
			}
...
class ExcepcioContinuarDemanant extends RuntimeException {

	public ExcepcioContinuarDemanant(String msg) {
		super(msg);
	}
}
